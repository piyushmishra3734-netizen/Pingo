...
export type CallEvent =
  ...
  /**
   * A line typed into the call itself.
   *
   * Not a conversation message, and deliberately not stored. A meeting chat is
   * where somebody says "go back a slide" while a screen is being shared; it
   * belongs to the call and dies with it, the way it does in every other
   * conferencing tool. Writing it to the thread would leave a transcript of
   * half-sentences in the middle of a real conversation nobody asked for.
   */
  | { type: 'call:chat'; message: CallChatMessage };

/** One line of in-call chat. Lives for the length of the call and no longer. */
export interface CallChatMessage {
  /** Generated by the sender, so a resend cannot be shown twice. */
  id: string;
  /** Who typed it. `undefined` is never valid - the room always knows. */
  userId: string;
  body: string;
...
export interface CallService {
   ...
   *
   * Optional for the same reason as `setScreenShare`: a mock has no room to say
   * it into. Resolves once sent; the sender's own copy arrives through
   * `call:chat` like everybody else's, so there is one path that puts a line on
   * screen rather than two that have to agree.
   */
  sendCallChat?(callId: string, body: string): Promise<void>;
